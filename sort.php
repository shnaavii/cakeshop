<?php
	...
	if(isset($_GET['sortedby'])){		
			...
			// $sql='';
			if($sort == 'low_to_high'){
				$sql = mysqli_query($conn, "SELECT * FROM `cakeshop` WHERE `price` BETWEEN  0  AND 100");
			}
			elseif($sort == 'high_to_low'){
				$sql = mysqli_query($conn,"SELECT * FROM `cakeshop` ORDER BY Price DESC");
			}
			else{
				$sql='';
				echo'there is no sort value';
			}
 ?>
<html>
	<head>
