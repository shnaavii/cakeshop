...
<?php 	
		...
		$i=0;
		$message='';
			if(mysqli_num_rows($sql) > 0 && isset($id)){
					...
					<form method="POST" class="shopc  me-4" style="float:left;">
						<div class="card mb-3  me-2 shopc " >
								...
								<div class="like-add bg-light position-absolute">
									<div class=" h-75 w-50 border m-1" style="position:relative;">
										<button class="btn cartadd" type="submit" name="add" >
											<input type="checkbox" class="plus-checkbox"  id="plus-checkbox'.$id.'">
											<label for="plus-checkbox'.$id.'" class="fa fa-plus fs-5 p-2 text-dark css-label-plusadd" ></label>
											<label for="plus-checkbox'.$id.'" class="fas fa-check fs-5 p-2 css-label-add" style="">	 </label>
										</button>
									</div>
									<div class="h-75 w-50 border m-1 position-relative"   >
										<button name="like" class="btn  ">
											<input type="checkbox" class="heart-checkbox"  id="heart-checkbox'.$id.'">
											<label for="heart-checkbox'.$id.'" class="heart fs-4 ">&#10084;</label>
										</button>
									</div>
								</div>
							...
							<div class="card-body text-center">
								...
								<input class="form-control" value='.$price.' id="'.$id.'" name="pricee" readonly="readonly"   type="hidden" >
...
?>				
			...
			<div class="searchdiv">
				<nav class="navbar bg-body-tertiary w-100">
					...
					<?php
						if(isset($_GET['s_submit']) && isset($_GET['searchh'])){
							$word=filter_var($_GET['searchh'],FILTER_SANITIZE_STRING);
